...
1. In order to prepare the data to test the SVM models you will have to run the following command `cp /projects/neurophon/TestsData/Model/whitenoise1/*.mat ~/neurophon/Octave`.
2. Then, run `>> load marks.mat`. This file contains all the marks generated by `processes_data_for_train_supervision_M` script.
3. Finally, run the following script from Matlab `>> processes_data_for_test_supervision_M(1,start_marks,end_marks)`. Such script will prepare the MRSTSA outputs and the EL outputs to test the SVM already trained models.
