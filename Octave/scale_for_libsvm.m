...
function C = scale_for_libsvm(A,arg_one,arg_two)

	# check argument's errors
	if (length(size(A)) != 2)
		string = "wrong A argument in scale_for_libsvm \n";
		string += "A's dimensionality must be 2.\n";
		error(string);
	endif
	if (!isa(A,'double'))
		string = "wrong A argument in scale_for_libsvm \n";
		string += "A's type must be double.\n";
		error(string);
	endif
	...
	if (nargin == 3)
		...
		if (length(arg_one) != 1 || length(arg_two) != 1)
			string = "wrong arg_one/two in scale_for_libsvm \n";
			string = [string, "argument's length must be 1.\n"];
			error(string);
		endif
		if (!isa(arg_one,'double') || !isa(arg_two,'double'))
			string = "wrong nim/max arguments in scale_for_libsvm \n";
	...
	elseif (nargin == 2)
		...
		if (!isa(arg_one,'struct'))
			string = "wrong arg_one in scale_for_libsvm \n";
			string = [string, "argument's type must be struct.\n"];
			error(string);
		endif
		if (!isa(arg_one.slopes,'double') || ...
		    length(size(arg_one.slopes)) != 2 || ...
		    rows(arg_one.slopes) != 1 || ...
		    columns(arg_one.slopes) != columns(A))
			string = "wrong arg_one.slopes in scale_for_libsvm \n";
			error(string);
		endif
		if (!isa(arg_one.intercept,'double') || ...
		    length(size(arg_one.intercept)) != 2 || ...
		    rows(arg_one.intercept) != 1 || ...
		    columns(arg_one.intercept) != columns(A))
			string = "wrong arg_one.intercept in scale_for_libsvm \n";
			error(string);
		endif
		C = arg_one.slopes.*A + arg_one.intercept;
		C(isnan(C))=0;
	else
		string = "number of arguments in scale_for_libsvm \n";
