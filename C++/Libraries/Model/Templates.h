...
template< typename T >
void	sample_vector( std::vector<T> vec, std::vector<T>& sample, std::size_t sampleSize )
{
...
template <typename T>
std::vector<T> get_elements(const std::vector<T> &v, const std::vector<std::size_t> &indexes) {

	std::vector<T> auxiliary;

	std::transform (indexes.begin(), indexes.end(), std::back_inserter(auxiliary),//
	[&v](std::size_t element) {return v.at(element);});
...
void    set_elements(std::vector<T> &v, const std::vector<std::size_t> &indexes, const V &val) {

	std::for_each (indexes.begin(), indexes.end(),//
	[&v, &val](std::size_t element) {v.at(element)=val;});
...
template <typename T, typename V>
std::vector<std::size_t>	less_than_indexes(const std::vector<T>& v, const V& limit)
{
...
template <typename T, typename V>
std::vector<std::size_t>	less_than_or_equal_to_indexes(const std::vector<T>& v, const V& limit)
{
	std::vector<std::size_t> results;
...
template <typename T>
std::vector<std::size_t>	get_indexes_from_value(const std::vector<T>& v, const T& value)
{
...
// returns true if there is -at least- one duplicate element
template< typename T >
bool	is_there_duplicate( std::vector<T> vec )
{
	assert(vec.size() > 0);

	std::sort(vec.begin(),vec.end());	
	return	(std::unique(vec.begin(), vec.end()) != vec.end());
} // end template is_there_duplicate



// finds the first coincident with value in a vector v and returns its index
// if there is no coincidence returns the number of elements in the vector
template <typename T>
std::size_t find_first_coincident_index( const std::vector<T>& v, const T value )
{
