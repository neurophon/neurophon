...
// File Name:		CorticalLayer6.cpp
// File Description:	CorticalLayer6 member-function definitions. This file contains implementations of the
//			member functions prototyped in CorticalLayer6.h.

#include <iostream>
#include <stdlib.h>							// calloc, exit, free
#include <float.h>
#include "CorticalLayer6.h"						// include definition of class CorticalLayer6

using namespace std;

// constructor initializes the dimensionality of the input and output vectors, this function validates the object too 
CorticalLayer6::CorticalLayer6( int l5InputDim, int inputDim, int outputDim, KohonenFeatureMapAbs &clusterOfNeurons )
...
void	CorticalLayer6::validateObject()
{
	...
	{
		cout << "CorticalLayer6 object inconsistence: inputDimensionality + extraInputDim = " << inputDim + extraInputDim << " and inputClusterDim = " << inputClusterDim << "\n" << endl;
		cout << "CorticalLayer6 object inconsistence: outputDimensionality = " << outputDim << " and outputClusterDim = " << outputClusterDim << "\n" << endl;
		exit( EXIT_FAILURE );
	}
...
void	CorticalLayer6::propagateSignals( neurotransmitters parameter )
{
	int	number;
	double	*vector;
	...
	level = CorticalLayer6::excitationLevel(number, vector);		// obtains the level of excitation to the cortical layer
	free(vector);
	vector = NULL;

	number = CorticalLayer6::getL5InputDimensionality();
	vector = CorticalLayer6::getL5InputVector();
	levelL5 = CorticalLayer6::excitationLevel(number, vector);		// obtains the level of excitation to the cortical layer
	free(vector);
	vector = NULL;

	if ( (level.active) && (levelL5.active) )				// if both inputs are active
	{
		CorticalLayer6::putOnInput();
...
levelOfExcitation	CorticalLayer6::excitationLevel( int numberOfComponents, double *vectorPointer )
{
	int i;
	double aux;
...
void	CorticalLayer6::putOnInput()
{
	...
	numberOfInputsFromL5 = CorticalLayer6::getL5InputDimensionality();		// gets the number of inputs from layer 5 in this cortical layer
	...
	inputVectorPointer = CorticalLayer6::getL5InputVector();			// gets the cortical layer's input from layer 5
...
void	CorticalLayer6::putOnOutput()
{
...
void	CorticalLayer6::setL5InputDimensionality( int dimension )
{
...
int	CorticalLayer6::getL5InputDimensionality()
{
...
void	CorticalLayer6::reservL5InputVector()
{
	int numberOfInputs;

	numberOfInputs = CorticalLayer6::getL5InputDimensionality();		// gets the number of extra inputs from layer 5
...
void	CorticalLayer6::setL5InputVector( double *inputs )
{
	int j, numberOfInputs;

	numberOfInputs = CorticalLayer6::getL5InputDimensionality();		// gets the number of inputs from layer 5
...
double	*CorticalLayer6::getL5InputVector()
{
	int j, numberOfInputs;
	double *vector;
